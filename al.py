...
frame_line = Frame(x, width=1200, height=6, bg = fline_col)
frame_line.grid(row=0, column=0)

frame_body=Frame(x, width=1200, height=400,bg='#B0C1D4')
...
rad1=Radiobutton(frame_body,font=('arial 10 bold'), value=1, text='Activate_Sound',bg='#B0C1D4',fg='#AD1414',command=activate_sound,variable=selected)
...
def sound_alarm():
	...
	rad2.place(x=340,y=230)
   

#Tk().withdraw() # to avoid showing the root window

message=Entry(frame_body,width=20,bg="#B0C1D4",borderwidth=3,fg="#AD1414")
message.place(x=250,y=290)
...
def msg():
    name = message.get()
    return messagebox.showinfo('message',f'{name}.')
...
mybutton=Button(frame_body,text="send msg",padx=10,pady=5,fg='#AD1414',bg="#B0C1D4",command=send_msg)
mybutton.place(x=390,y=290)
